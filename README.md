...
| `events`                    | `[]`                                                                             | `Array<{ date: Moment, date2: Moment }>`  | Events passed into the calendar. These objects will be injected into the correct array by date. `date2` is optional and only valid for events spanning a date range. `date` can also be specified as `date1`.                                                                           |
